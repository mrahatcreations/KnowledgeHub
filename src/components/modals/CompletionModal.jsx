...
import { Star, Trophy, RefreshCw, ArrowRight, BookOpen, Volume2, CheckCircle2, XCircle, AlertCircle, Sparkles, Award } from 'lucide-react';
...
export default function CompletionModal({ 
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-md flex items-center justify-center p-3 sm:p-4 select-none animate-fadeIn">
      <div className="bg-white rounded-3xl max-w-md w-full shadow-2xl border border-slate-100 animate-pop overflow-hidden flex flex-col max-h-[92vh]">
        {/* Header / Score Banner */}
        <div className={`p-5 sm:p-6 text-center text-white relative ${
          isMastered 
            ? 'bg-gradient-to-b from-amber-500 via-amber-600 to-amber-700' 
            : 'bg-gradient-to-b from-indigo-600 via-indigo-700 to-slate-900'
        }`}>
          {/* Trophy / Star Avatar */}
          <div className="w-14 h-14 rounded-2xl bg-white/20 backdrop-blur-xs flex items-center justify-center mx-auto mb-2.5 shadow-inner">
            {isMastered ? (
          ...
          {/* 10-Star Display Grid / Row */}
          <div className="flex flex-wrap items-center justify-center gap-1 sm:gap-1.5 my-3 px-2">
        ...
        {mistakes.length > 0 && (
          <div className="flex border-b border-slate-200 bg-slate-50 px-3 pt-2 shrink-0">
            <button
        ...
        <div className="p-4 sm:p-5 overflow-y-auto flex-1 space-y-4">
          {activeTab === 'mistakes' && mistakes.length > 0 ? (
            <div className="space-y-3">
              ...
              {mistakes.map((m, idx) => (
                <div key={idx} className="p-3.5 sm:p-4 rounded-2xl bg-slate-50 border border-slate-200 text-left space-y-2 shadow-xs">
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      ...
                      className="p-1.5 text-slate-400 hover:text-indigo-600 rounded-full hover:bg-white transition active:scale-95"
                      title="উচ্চারণ শুনুন"
                    >
                  ...
                  {m.userAnswer && (
                    <div className="flex items-start space-x-1.5 text-xs text-rose-600 bg-rose-50 px-2.5 py-1.5 rounded-lg border border-rose-200 break-words">
                  ...
                  <div className="flex items-start space-x-1.5 text-xs text-emerald-700 bg-emerald-50 px-2.5 py-1.5 rounded-lg border border-emerald-200 break-words">
                  ...
                  {m.explanation && (
                    <p className="text-[11px] text-slate-600 bg-white p-2.5 rounded-lg border border-slate-100 leading-relaxed break-words">
                      {m.explanation}
                    </p>
                  )}
                </div>
            ...
            <div className="space-y-4 py-1">
              ...
              {/* Status Message */}
              {isMastered ? (
                <div className="p-4 rounded-2xl bg-emerald-50 border border-emerald-200 text-emerald-900 text-xs font-semibold flex items-start space-x-2 text-left">
                ...
                <div className="p-4 rounded-2xl bg-amber-50 border border-amber-200 text-amber-900 text-xs font-semibold leading-relaxed text-left">
        ...
        {/* Footer Actions */}
        <div className="p-4 bg-slate-50 border-t border-slate-200 flex flex-col space-y-2 shrink-0">
          {isMastered ? (
            <button
              onClick={onNextLevel}
              className="w-full py-3.5 px-6 rounded-2xl bg-emerald-600 hover:bg-emerald-700 text-white font-black text-sm flex items-center justify-center space-x-2 transition shadow-md hover:shadow-lg active:scale-95"
            >
            ...
            <button
              ...
              className="w-full py-3.5 px-6 rounded-2xl bg-indigo-600 hover:bg-indigo-700 text-white font-black text-sm flex items-center justify-center space-x-2 transition shadow-md hover:shadow-lg active:scale-95"
            >
