...
export default function TrueFalseSwipeStage({ stage, onSubmitAnswer, isSecondChance }) {
  ...
  return (
    <div className="w-full max-w-[300px] sm:max-w-sm mx-auto flex flex-col items-center space-y-4 sm:space-y-5 animate-pop select-none">
      ...
      <div className="relative w-full min-h-[290px] sm:min-h-[320px] flex items-center justify-center pt-1 sm:pt-2">
        {/* Background Deck Card 2 (Deepest) */}
        <div 
          className="absolute w-[90%] h-[92%] rounded-2xl sm:rounded-3xl bg-slate-800/60 border border-slate-700/50 shadow-md transition-transform duration-300 pointer-events-none"
          style={{
            transform: `translateY(14px) scale(${0.92 + (isDragging ? 0.02 : 0)})`,
            opacity: 0.5
          }}
        />
        ...
        <div 
          className="absolute w-[95%] h-[96%] rounded-2xl sm:rounded-3xl bg-slate-800/90 border border-slate-700/80 shadow-lg transition-transform duration-300 pointer-events-none"
          style={{
            transform: `translateY(7px) scale(${0.96 + (isDragging ? 0.02 : 0)})`,
            opacity: 0.8
          }}
        />
        ...
        <div
          ...
          className={`relative w-full min-h-[280px] sm:min-h-[310px] p-4 sm:p-6 rounded-2xl sm:rounded-3xl bg-gradient-to-b from-white to-slate-50 border-3 flex flex-col justify-between cursor-grab active:cursor-grabbing ${
        ...
        >
          ...
          <div
            ...
            className="absolute top-4 right-4 sm:top-5 sm:right-5 z-20 px-3 py-1.5 sm:px-4 sm:py-2 rounded-xl sm:rounded-2xl border-[3px] sm:border-[3.5px] border-emerald-500 bg-emerald-500/15 backdrop-blur-xs text-emerald-600 font-black flex items-center space-x-1.5 sm:space-x-2 pointer-events-none transition-all duration-75"
          >
            <Check className="w-5 h-5 sm:w-6 sm:h-6 stroke-[3.5]" />
            <div className="flex flex-col items-start leading-none">
              <span className="text-base sm:text-lg tracking-wider font-black">TRUE</span>
              <span className="text-[10px] sm:text-[11px] font-bold text-emerald-700">সত্য</span>
            </div>
          </div>
          ...
          <div
            ...
            className="absolute top-4 left-4 sm:top-5 sm:left-5 z-20 px-3 py-1.5 sm:px-4 sm:py-2 rounded-xl sm:rounded-2xl border-[3px] sm:border-[3.5px] border-rose-500 bg-rose-500/15 backdrop-blur-xs text-rose-600 font-black flex items-center space-x-1.5 sm:space-x-2 pointer-events-none transition-all duration-75"
          >
            <X className="w-5 h-5 sm:w-6 sm:h-6 stroke-[3.5]" />
            <div className="flex flex-col items-start leading-none">
              <span className="text-base sm:text-lg tracking-wider font-black">FALSE</span>
              <span className="text-[10px] sm:text-[11px] font-bold text-rose-700">মিথ্যা</span>
            </div>
          </div>
          ...
          <div className="flex items-center justify-between z-10">
            <div className="flex items-center space-x-1.5 sm:space-x-2">
              <span className="px-2.5 sm:px-3 py-0.5 sm:py-1 bg-indigo-50 border border-indigo-200/80 text-indigo-700 text-[11px] sm:text-xs font-extrabold rounded-full uppercase tracking-wider shadow-xs">
                {stage?.item?.pos || 'VOCABULARY'}
              </span>
              {isSecondChance && (
                <span className="px-2 sm:px-2.5 py-0.5 sm:py-1 bg-amber-100 border border-amber-300 text-amber-800 text-[10px] sm:text-[11px] font-black rounded-full animate-pulse flex items-center space-x-1 shadow-xs">
            ...
            <button
              onClick={handleSpeak}
              type="button"
              className={`p-2 sm:p-2.5 rounded-xl sm:rounded-2xl transition-all shadow-xs flex items-center justify-center ${
                isSpeaking 
          ...
          <div className="text-center py-3 sm:py-4 px-1 sm:px-2 my-auto z-10">
            <div className="inline-block relative max-w-full">
              <h2 className="text-2xl sm:text-3xl md:text-4xl font-black text-slate-900 tracking-tight break-words">
                {stage?.item?.word}
              </h2>
              {stage?.item?.phonetic && (
            ...
            <div className="my-2.5 sm:my-3.5 h-1 w-10 sm:w-12 bg-gradient-to-r from-indigo-300 via-indigo-500 to-indigo-300 mx-auto rounded-full" />

            {/* Bengali Meaning Statement Challenge */}
            <div className="bg-slate-100/80 border border-slate-200/80 rounded-xl sm:rounded-2xl p-2.5 sm:p-3.5 shadow-inner">
              <p className="text-[11px] sm:text-xs text-slate-500 font-semibold mb-0.5 sm:mb-1">প্রদর্শিত অর্থ কি সঠিক?</p>
              <p className="text-base sm:text-lg md:text-xl font-extrabold text-slate-800 leading-snug break-words">
          ...
          <div className="flex items-center justify-between text-[10px] sm:text-[11px] font-bold text-slate-400 px-1 pt-2 border-t border-slate-100 z-10">
            <div className="flex items-center space-x-1 text-rose-500/80">
              <ArrowLeft className="w-3 h-3 sm:w-3.5 sm:h-3.5 animate-pulse" />
            ...
            <div className="flex items-center space-x-1 text-emerald-600/80">
              <span>ডানে: সত্য</span>
              <ArrowRight className="w-3 h-3 sm:w-3.5 sm:h-3.5 animate-pulse" />
            </div>
          </div>
        </div>
      </div>

      {/* 3D Tactile Action Controls (Duolingo/Candy Crush Style - 50/50 Grid) */}
      <div className="grid grid-cols-2 gap-3 w-full pt-1">
        ...
        <button
          ...
          disabled={isAnswered}
          className="game-btn-3d game-btn-rose w-full py-3 sm:py-3.5 px-3 sm:px-4 rounded-xl sm:rounded-2xl text-white font-black flex items-center justify-center space-x-2 sm:space-x-2.5 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed shadow-md"
        >
          <div className="p-1 sm:p-1.5 rounded-lg sm:rounded-xl bg-white/20 shrink-0">
            <X className="w-4 h-4 sm:w-5 sm:h-5 stroke-[3.5]" />
          </div>
          <div className="flex flex-col items-start leading-tight">
            <span className="text-[10px] sm:text-[11px] font-bold uppercase tracking-wider opacity-90">মিথ্যা</span>
        ...
        <button
          ...
          disabled={isAnswered}
          className="game-btn-3d game-btn-emerald w-full py-3 sm:py-3.5 px-3 sm:px-4 rounded-xl sm:rounded-2xl text-white font-black flex items-center justify-center space-x-2 sm:space-x-2.5 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed shadow-md"
        >
          <div className="p-1 sm:p-1.5 rounded-lg sm:rounded-xl bg-white/20 shrink-0">
            <Check className="w-4 h-4 sm:w-5 sm:h-5 stroke-[3.5]" />
          </div>
          <div className="flex flex-col items-start leading-tight">
            <span className="text-[10px] sm:text-[11px] font-bold uppercase tracking-wider opacity-90">সত্য</span>
