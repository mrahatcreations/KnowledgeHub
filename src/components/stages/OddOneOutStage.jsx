...
export default function OddOneOutStage({ stage, onSubmitAnswer, isSecondChance }) {
  ...
  return (
    <div className="w-full max-w-xl mx-auto flex flex-col space-y-4 sm:space-y-5 animate-pop select-none">
      ...
      <div 
        className={`relative overflow-hidden p-5 sm:p-6 rounded-3xl bg-white border-2 text-left shadow-lg transition-all duration-300 ${
        ...
        <div className="relative z-10">
          ...
          <div className="flex items-center justify-between gap-2 mb-3">
            <div className="flex items-center space-x-2">
              <div className="w-8 h-8 rounded-xl bg-gradient-to-tr from-purple-600 to-indigo-500 text-white flex items-center justify-center shadow-md shadow-purple-500/30 animate-pulse">
                <Sparkles className="w-4 h-4" />
              </div>
              <span className="px-3 py-1 bg-purple-100/80 border border-purple-200 text-purple-800 text-xs font-black rounded-full uppercase tracking-wider">
                বেমানান শব্দ চ্যালেঞ্জ
              </span>
            </div>

            {effectiveSecondChance ? (
              <span className="px-3 py-1 bg-amber-100 border border-amber-300 text-amber-900 text-xs font-black rounded-full animate-pulse flex items-center gap-1 shadow-xs">
                <Flame className="w-3.5 h-3.5 text-amber-600" />
                ২য় সুযোগ (০ স্টার)
              </span>
            ) : (
              <span className="text-[11px] sm:text-xs font-bold text-slate-400 bg-slate-100/90 px-2.5 py-1 rounded-full border border-slate-200/60 flex items-center gap-1">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-3.5">
        {stage.options.map((opt, i) => {
          ...
          let cardStyle = 'border-slate-200 bg-white text-slate-800 shadow-[0_5px_0_#cbd5e1] hover:shadow-[0_6px_0_#cbd5e1,0_0_20px_rgba(168,85,247,0.22)] hover:border-purple-400 hover:-translate-y-0.5 active:translate-y-1 active:shadow-[0_1px_0_#cbd5e1]';
          ...
          if (isWrong) {
            ...
            cardStyle = 'border-emerald-500 bg-emerald-50/95 text-emerald-950 shadow-[0_5px_0_#059669,0_0_25px_rgba(16,185,129,0.35)] animate-pulse-glow translate-y-0.5';
          ...
          return (
            <button
              ...
              className={`group relative p-4 sm:p-4.5 rounded-2xl sm:rounded-3xl border-2 font-bold text-left transition-all duration-150 flex items-center justify-between gap-3 ${cardStyle}`}
            >
              ...
              <div className="flex items-center space-x-1.5 shrink-0">
                <span
                  ...
                  title="উচ্চারণ শুনুন"
                  className="p-1.5 rounded-lg text-slate-400 hover:text-purple-600 hover:bg-purple-100/70 transition active:scale-90"
                >
                  <Volume2 className="w-4 h-4" />
                </span>

                {isWrong && (
                  <XCircle className="w-5 h-5 text-rose-500 shrink-0 animate-pop" />
                )}
                {isSelected && (
                  <CheckCircle2 className="w-5 h-5 text-emerald-600 shrink-0 animate-pop" />
                )}
              </div>
            </button>
          );
      ...
      {showExplanation && stage.explanation && (
        <div className="p-4 sm:p-5 rounded-2xl bg-indigo-50/90 border-2 border-indigo-200/90 shadow-md animate-pop text-left">
          <div className="flex items-center space-x-2 text-indigo-900 font-extrabold text-xs sm:text-sm mb-1.5">
